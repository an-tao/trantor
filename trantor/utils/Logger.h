...
#define LOG_WARN_IF(cond)  \
    if(cond)Tensor::Logger(__FILE__, __LINE__, Tensor::Logger::kWarn).stream()
#define LOG_ERROR_IF(cond)  \
    if(cond)Tensor::Logger(__FILE__, __LINE__, Tensor::Logger::kError).stream()
#define LOG_FATAL_IF(cond) \
    if(cond)Tensor::Logger(__FILE__, __LINE__, Tensor::Logger::kFatal).stream()
...
#define DLOG_INFO                                               \
    ...
    Tensor::Logger(__FILE__, __LINE__).stream()
#define DLOG_WARN \
    if(0)Tensor::Logger(__FILE__, __LINE__, Tensor::Logger::kWarn).stream()
#define DLOG_ERROR \
    if(0)Tensor::Logger(__FILE__, __LINE__, Tensor::Logger::kError).stream()
#define DLOG_FATAL \
    if(0)Tensor::Logger(__FILE__, __LINE__, Tensor::Logger::kFatal).stream()
